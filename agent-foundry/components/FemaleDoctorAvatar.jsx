...
export default function FemaleDoctorAvatar({ state = "idle" }) {
  return (
    <div className="relative h-full w-full rounded-3xl overflow-hidden glass-panel-medical">
      ...
      {/* State glow border */}
      <div className={`absolute inset-0 rounded-3xl shadow-medical-glow animate-pulse-slow ${
        state === "speaking" ? "shadow-medical-glow" :
      ...
      <div className="absolute top-3 right-3 z-20">
        <div className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium backdrop-blur-xl ${
          state === "speaking" ? "bg-medical-success/90 text-white border border-medical-success" :
          ...
          <span className={`status-dot h-2 w-2 ${
            state === "speaking" ? "status-dot-active" : "status-dot-idle"
          }`} />
